...
UI_TICK_MS = 150  # UI refresh cadence (smaller feels snappier)
...
def update_transcript_UI(transcriber, transcript_textbox, responder, response_textbox, freeze_state):
    transcript_string = transcriber.get_transcript()
    write_in_textbox(transcript_textbox, transcript_string)

    # If transcript changed, show placeholder immediately (UX: feels instant)
    if not freeze_state[0] and transcriber.transcript_changed_event.is_set():
        response_textbox.configure(state="normal")
        write_in_textbox(response_textbox, "Thinking…")
        response_textbox.configure(state="disabled")

    transcript_textbox.after(UI_TICK_MS, update_transcript_UI, transcriber, transcript_textbox, responder, response_textbox, freeze_state)
...
def update_response_UI(responder, textbox, freeze_state):
    ...
    textbox.after(UI_TICK_MS, update_response_UI, responder, textbox, freeze_state)
...
def create_ui_components(root):
    ...
    update_interval_slider_label = ctk.CTkLabel(root, text="", font=("Arial", 12), text_color="#FFFCF2")
    update_interval_slider_label.grid(row=2, column=1, padx=10, pady=3, sticky="nsew")

    # Allow sub-second updates (0.5s steps)
    update_interval_slider = ctk.CTkSlider(root, from_=0.5, to=6.0, width=300, height=20, number_of_steps=11)
    update_interval_slider.set(1.0)
...
def main():
    ...
    transcribe = threading.Thread(target=transcriber.transcribe_audio_queue, args=(audio_queue,), daemon=True)
    transcribe.start()

    responder = GPTResponder()

    # Update responder interval ONLY when slider changes (not every UI tick)
    def on_slider_change(value):
        interval = float(value)
        update_interval_slider_label.configure(text=f"Update interval: {interval:.1f} seconds")
        responder.update_response_interval(interval)

    update_interval_slider.configure(command=on_slider_change)
    on_slider_change(update_interval_slider.get())

    respond = threading.Thread(target=responder.respond_to_transcriber, args=(transcriber,), daemon=True)
    respond.start()
    ...
    update_transcript_UI(transcriber, transcript_textbox, responder, response_textbox, freeze_state)
